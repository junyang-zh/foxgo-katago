...
4. Start at any point in an even game. Three matching observations import the current position into KataGo and the main board. The FoxGo move counter determines the turn (Black after an even count, White after an odd count). Subsequent moves and captures are tracked normally. Earlier history, captures and ko restrictions are unknown; the timeline and SGF begin with an explicit setup position. Handicap attachment remains unsupported.
...
A click is committed to KataGo only after its stone and captures are seen. A fast opponent reply can confirm both consecutive legal moves. The input boundary checks the physical cursor position before sending button events. On the next observation cycle, the pointer is parked over the title bar to avoid FoxGo's hover marker. Screen polling and three-frame confirmation add about one second; this connector does not read clocks, so use a conservative engine search limit (for example 1–2 seconds) and avoid very fast time controls.
